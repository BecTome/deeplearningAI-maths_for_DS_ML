...
def test_matrix(target_P, target_X0, target_X1):
    ...
    test_cases = [
        {
            ...
            "expected": {
                ...
                "X1": np.array([[0.25], [0.25], [0.25], [0], [0.25]]),
            },
        },
    ]
    ...
    for test_case in test_cases:
        ...
        try:
            assert np.allclose(target_P, test_case["expected"]["P"])
            successful_cases += 1
        except:
            failed_cases.append(
                {
                    "name": test_case["name"],
                    "expected": np.nonzero(
                        np.logical_not(np.isclose(target_P, test_case["expected"]["P"]))
                    ),
                    "got": target_P,
                }
            )
        ...
        try:
            assert target_X1.shape == test_case["expected"]["X1"].shape
            successful_cases += 1
        except:
            failed_cases.append(
                {
                    "name": test_case["name"],
                    "expected": test_case["expected"]["X1"].shape,
                    "got": target_X1.shape,
                }
            )
            print(
        ...
        try:
            assert np.allclose(target_X1, test_case["expected"]["X1"])
            successful_cases += 1
        except:
            failed_cases.append(
                {
                    "name": test_case["name"],
                    "expected": test_case["expected"]["X1"],
                    "got": target_X1,
                }
            )
            print(
